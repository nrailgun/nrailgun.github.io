...
$C = AB$ 在工程上一般是怎么算的呢？可以参考 [How to Optimize GEMM](https://github.com/flame/how-to-optimize-gemm/wiki)。

假定 $A$ 的 size 为 $m \times k$，$B$ 的 size 为 $k \times n$，$C$ 的 size 为 $m \times n$。
                                       ...
                                       double *c, int ldc )
{
  ...
  for ( j=0; j<n; j+=4 ){        /* Loop over the columns of C, unrolled by 4 */
    for ( i=0; i<m; i+=4 ){        /* Loop over the rows of C */
      ...
      // 之所以 copy 一遍，是为了把 A 从 col maj 改成 row maj。
